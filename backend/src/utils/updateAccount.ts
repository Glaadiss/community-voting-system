...
import { allowAdmin } from '../services/auth';
import { validateEmailFormat, validatePassword, validateEmail } from './validator';
...
function updateAccount({ updateFunction, validators }) {
    return async (parent, { data, where }, context: Context, info) => {
        if (!isUserAllowed(context) || (!isAdminAllowed(context) && !(where.id === context.user.id || where.email === context.user.email)))
